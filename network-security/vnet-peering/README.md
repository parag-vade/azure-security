# Vnet Peering
- private connectivity between two or more virtual networks- traffic never traverses the public interne
- Differnt than VPN
    - Vnet peering is only between Azure Vnets. VPN is for connecting Azure with on-prem or other clouds
    - Vnet peering connection is not encrypted but uses Azure backbone network. VPN is encrypted using IPSe/IKE but traffic flows over internet
